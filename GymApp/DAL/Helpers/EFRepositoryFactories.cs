    ...
    public class EFRepositoryFactories : IDisposable
    {
        ...
        private static IDictionary<Type, Func<IDbContext, object>> GetCustomFactories()
        {
            return new Dictionary<Type, Func<IDbContext, object>>
            {
                ...
                {typeof (IUserInPlanRepository), dbContext => new UserInPlanRepository(dbContext)},
                {typeof (IUserRoleInPlanRepository), dbContext => new UserRoleInPlanRepository(dbContext)},
                {typeof (IWorkoutInPlanRepository), dbContext => new WorkoutInPlanRepository(dbContext)},
